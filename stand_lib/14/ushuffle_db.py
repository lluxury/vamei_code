#!/usr/bin/env python
import os
#from random import randrang as rand
from random import randrange as rand

COLSIZ = 10
#FIELDs = ('login', 'userid', 'projid')
FIELDS = ('login', 'userid', 'projid')
#PDBMSs = {'s': 'sqlite', 'm': 'mysql', 'g': 'gadfly'}
...
def connect(db):
    ...
    if db == 'sqlite':
        ...
        if not os.path.isdir(dbDir):
            os.mkdir(dbDir)
        #cnx = sqlite3.connect(os.path.join(dbDir, DBNAME))
...
def main():
    ...
    cxn = connect(db)
    if not cxn:
        #print 'ERROR: %r not suppported or unreachable, exiting' % db
